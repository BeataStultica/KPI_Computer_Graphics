...
public class TriangleTest {
    ...
    void notIntersectsWith() {
        Ray ray = new Ray(new Vector(1, 0, 0), new Point(5, 0, 0));
        assertNull(triangle.intersectionWith(ray));
    }
    @Test
    void IntersectInVerticle() {
        Ray ray = new Ray(new Vector(-1, 0, 0), new Point(5, 15, 0));
        assertNotNull(triangle.intersectionWith(ray));
    }
    @Test
    void NotIntersectNearVerticle() {
        Ray ray = new Ray(new Vector(-1, 0, 0), new Point(5, 15.1, 0));
        assertNull(triangle.intersectionWith(ray));
    }
    @Test
    void IntersectInEdge() {
        Ray ray = new Ray(new Vector(-1, 0, 0), new Point(5, 15/2*Math.sqrt(2), 15/2*Math.sqrt(2)/2));
        assertNotNull(triangle.intersectionWith(ray));
    }
    @Test
    void NotIntersectNearEdge() {
        Ray ray = new Ray(new Vector(-1, 0, 0), new Point(5, 15/2*Math.sqrt(2)+0.5, 15/2*Math.sqrt(2)+0.5));
        assertNull(triangle.intersectionWith(ray));
    }
    @Test
    void NotIntersectParallel() {
        Ray ray = new Ray(new Vector(0, -1, -1), new Point(5, 0, 0));
        assertNull(triangle.intersectionWith(ray));
    }
}
